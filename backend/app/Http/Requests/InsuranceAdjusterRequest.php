...
class InsuranceAdjusterRequest extends FormRequest
{
    /**
    ...
    public function rules(): array
{
    ...
    return [
        ...
        'insurance_company_id' => [
            ...
            // Añade cualquier otra validación necesaria para insurance_company_id
            //function ($attribute, $value, $fail) use ($isUpdateRequest) {
                //if ($isUpdateRequest) {
                    // Obtiene el ID del ajustador actual basado en el UUID
                    //$currentAdjusterId = InsuranceAdjuster::where('uuid', $this->route('uuid'))->value('id');

                    //if ($currentAdjusterId) {
                        // Verifica si el mismo `user_id` y `insurance_company_id` están ya asignados a otro ajustador
                        //$existingAdjuster = InsuranceAdjuster::where('insurance_company_id', $value)
